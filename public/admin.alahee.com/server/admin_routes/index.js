            ...
            if (req.files.productDescriptionFiles) {
              if (Array.isArray(req.files.productDescriptionFiles)) {
                req.files.productDescriptionFiles.map(function (file, index) {
                    ...
                    let file_signature = file.data.toString('hex', 0, 4)
                    let file_extension = file.name.slice( ( ( file.name.lastIndexOf('.') - 1 ) >>> 0 ) + 2 )
                    if ((file.size / (1024 * 1024)) > allowed_file_size) return res.status(400).json({ msg: 'File too large' })      
                    if ( array_of_allowed_file_signatures.includes(file_signature) ) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })
                    if (!array_of_allowed_files.includes(file_extension) || !array_of_allowed_file_types.includes(file.mimetype)) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })      
                ...
                let file_signature = productDescriptionFiles.data.toString('hex', 0, 4)
                let file_extension = productDescriptionFiles.name.slice( ( ( productDescriptionFiles.name.lastIndexOf('.') - 1 ) >>> 0 ) + 2 )
                if ((productDescriptionFiles.size / (1024 * 1024)) > allowed_file_size) return res.status(400).json({ msg: 'File too large' })      
                if ( array_of_allowed_file_signatures.includes(file_signature) ) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })
                if (!array_of_allowed_files.includes(file_extension) || !array_of_allowed_file_types.includes(productDescriptionFiles.mimetype)) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })
          ...
          if (!req.body.colorFiles) {
            ...
            if (Array.isArray(productFilesArray)) {
              ...
              let productFiles = req.files.colorFiles;
                ...
                let file_signature = productFiles.data.toString('hex', 0, 4)
                let file_extension = productFiles.name.slice( ( ( productFiles.name.lastIndexOf('.') - 1 ) >>> 0 ) + 2 )
                if ((productFiles.size / (1024 * 1024)) > allowed_file_size) return res.status(400).json({ msg: 'File too large' })      
                if ( array_of_allowed_file_signatures.includes(file_signature) ) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })
                if (!array_of_allowed_files.includes(file_extension) || !array_of_allowed_file_types.includes(productFiles.mimetype)) return res.status(400).json({ msg: 'Supported image files are jpeg, jpg, and png' })
...
routes.get('/customer_address', async function (req, res) {
  try {
    ...
    return res.send({ success: true, customer_address: customer_address });
  } catch (e) {
    console.log(e)
    return res.send({ success: false, message: 'Something went wrong!', error: e });
  }
