...
const getCategoryWiseProductList = async (leafChildrenArr, cat_id) => {
  ...
  for (const { id, parent_category_id } of leafChildrenArr) {
    ...
    for (const product of products) {
      const discountAmount = getDiscountByProductId(discountArr, product.product_id);
...
const showProductListBySlug = async (slug) => {
  // clicked on parent category
  const cat_id = await getCategoryIdFromSlug(slug);
...
const getDiscountByProductId_20122021 = (discountArr, product_id) => {
...
const getDiscountByProductId = (discountArr, product_id) => {
    ...
    for (const item of discountArr) {
        ...
        itemArr.forEach(({ id, discount }) => {
            if ( id == product_id ) {
                // let text = JSON.stringify(discount);
                // let result = text.includes("%")
                // if(result) discountAmount = text;
                // else discountAmount += parseInt(discount);
