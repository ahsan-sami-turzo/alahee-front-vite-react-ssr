...
const temp_wish = require("./tempSellWishList.js"); 
...
// const upload_path = `${__dirname}/../../alaheeAdmin/public/upload/customerPhoto/`;
...
console.log('after db connection on frontend');
    ...
    db_products.length > 0 &&
      db_products.forEach((element) => {
        products.push({
          ...
          product_url: `${process.env.REACT_APP_FRONTEND_URL}/product-details/${element.id}`,
          product_image: `${process.env.REACT_APP_FRONTEND_URL}/${element.home_image}`,
        });
      });
...
router.get("/company_info", async (req, res) => {
  ...
  for (const companyInfo of company_infos) {
    if (companyInfo.name === "alahee")
      result = { ...result, alahee: companyInfo };
  }
});
...
router.get("/top_main_banners", async (req, res) => {
  try {
    const mainBanners = await query(
      `SELECT id, name, image, url FROM banner WHERE softDel=0 AND status=1 order by rand() limit 6`
    );
...
router.get("/feature_product_list", async (req, res) => {
  try {
    ...
    for (const { feature_id, title, code } of feature_names) {
      ...
      console.log('feature_product_list', products);
...
router.get("/productDetails/:productId", async (req, res) => {
  ...
  try {
    ...
    // Number array
    // if (product_specification[0].hasOwnProperty("Number")) {
    if (product_specification.hasOwnProperty("Number")) {
      const { Number } = product_specification[0];
      productDetails.sizes = await Promise.all(
        Number.map(async (id) => {
          const data = await query(
            `SELECT id, size, size_type_id FROM size_infos WHERE id=${id.id} AND softDel=0 AND status=1`
          );
          return { ...data[0] };
        })
      );
    ...
    // product_full_description
    // desc = (stringToJson(JSON.stringify(product_full_description)))
    // const product_description = ( JSON.parse( JSON.stringify(desc[0]) ) )
    // console.log( 'product_description', product_description )
    
    const product_description = (product_full_description) ? JSON.parse(product_full_description) : "";
...
router.post("/payOrder", async (req, res) => {
  ...
  try {
    ...
    console.log('...tempSells...', tempSells)
    ...
    if (saleRecord.length > 0) {
      ...
      var newBillNo = "ALH-" + year + "-" + newBillPadding;
    } else {
    ...
    for (const tempSellItem of tempSells) 
    {
      ...
      for (const j in discountDetail) {
        if (tempSellItem.productId === discountDetail[j].productId) {
            let disAmt = ( discountDetail[j].amount && discountDetail[j].amount > 0 ) ? discountDetail[j].amount : 100
            discountAmount2 += (discountDetail[j].amount / 100) * totalAmount;
        }
      }
      ...
      console.log('check...', itemInventory, tempSellItem.quantity)
      
      if (itemInventory >= tempSellItem.quantity) {
        billPaddingInt = parseInt(tempSellItem.vendorId);
        newBillPadding = billPaddingInt.toString().lpad("0", 5);
        var vendorBillNo = newBillNo + "-" + newBillPadding;
      
        const insertData = await query(
          "insert into sales_details(customerId, salesBillId, vendor_sales_bill_no, vendor_id, entry_by, entry_user_type, productId, colorId, sizeId, sales_product_quantity,unitPrice,total_amount,customer_payable_amount,discounts_amount) VALUES('" +
            customerId +
            "','" +
            salesId +
            "','" +
    ...
    //  .status(404)
    //   .send({ error: true, data: false, message: error.message });
      .send({ error: true, data: false, message: "Error! Can not purchase right now. Please come back later.", error_message : error.message });
  }
});

const url1 = "http://ecomservice.alahee.com/api";
const url2 = "microservice.alahee.com/api";
      ...
      if (cartIds.includes(specific) === true) {
        for (const k in cartProductQty) {
          if (cartProductQty[k].productId == specific) {
            ...
            const productPrice = await query(`SELECT productPrice FROM products WHERE id = ${cartProductQty[k].productId}`);
              
            // discountAmount =
            //   discountAmount +
            //   parseInt(parsedArr[j].discount) * cartProductQty[k].quantity;
            //   console.log( 'parsedArr[j].discount ', parsedArr[j].discount, cartProductQty[k], productPrice[0].productPrice)
              
               if( parsedArr[j].discount.toString().includes("%") ) discountAmount += Math.floor( parseInt(parsedArr[j].discount) * productPrice[0].productPrice / 100 ) * cartProductQty[k].quantity
               else discountAmount += parseInt(parsedArr[j].discount) * cartProductQty[k].quantity
          }
        }
        discountDetail.push({
          id: discounts[i].id,
...
// Shipping Amount API
router.post("/shipping_amount", async (req, res, next) => {
    try {
        const cartProducts = req.body.cartProducts;
        const cartIds = [];
        const cartProductQty = [];
        ...
        if( cartIds.length > 0 ){
            const deliver_and_charge = await query(
                // `SELECT MAX(deliver_and_charge.charge) as deliver_and_charge FROM deliver_and_charge 
                // LEFT JOIN products ON products.category_id = deliver_and_charge.category_id 
                // WHERE products.id IN ( ${cartIds} ) AND deliver_and_charge.softDel=0;`
                `SELECT MAX(deliver_and_charge.charge) as deliver_and_charge FROM deliver_and_charge 
                WHERE deliver_and_charge.category_id IN 
                ( SELECT category.parent_category_id FROM products join category on category.id = products.category_id where products.id = ${cartIds} )`
            );
            // const deliver_and_charge = await query( 
            //     "SELECT MAX(deliver_and_charge.charge) as deliver_and_charge FROM deliver_and_charge LEFT JOIN products ON products.category_id = deliver_and_charge.category_id WHERE deliver_and_charge.softDel=0" +
            //     " AND products.id IN " + "(" + cartIds + ")" 
            // );
            
            console.clear()
            console.log( 'cartIds... ', cartIds, deliver_and_charge)
            
            shipping_amount = parseInt(deliver_and_charge[0].deliver_and_charge)
        } 
  ...
  try {
    await query(`
      ...
      SET
        ...
        thana = ${JSON.stringify(thana)},
        postOffice = ${JSON.stringify(postOffice)},
        area = ${JSON.stringify(area)},
        zipcode = ${JSON.stringify(postcode)}
      WHERE
        id = ${customerId};
    `);
...
router.get("/get_order_history/:customer_id", async (req, res) => {
  ...
  try {
    ...
    SELECT
      ...
      products.brand_name,
      products.home_image,
      sales_details.sales_product_quantity,
      sales_details.unitPrice,
      sales_details.total_amount,
      sales_details.isAcceptedByVendor,
      sales_details.delivery_status
    FROM
        sales 
    join
        sales_details                              
...
router.get("/productListBySlug/:slug", async (req, res) => {
  const { slug } = req.params;
  try {
    const data = await showProductListBySlug(slug);
    res.json([...data]);
  } catch (e) {
    console.error(e);
    res.status(500).send("Server Error");
...
router.get("/getCompanyInfo", async (req, res) => {
  try {
    const data = await query(`
      SELECT 
        name, logo, logo_mob, qrcode, email, phone, telephone, website, address 
      FROM 
        gnr_company
      WHERE 
        status = 1
