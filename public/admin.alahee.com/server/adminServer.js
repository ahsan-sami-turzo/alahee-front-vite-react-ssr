...
var key = fs.readFileSync(__dirname + '/certifacte/back/server.key');
var cert = fs.readFileSync(__dirname + '/certifacte/back/server.cert');
var credentials = {
  key: key,
  cert: cert
};
...
app.use(session({ key: 'username', secret: 'alahee', resave: false, saveUninitialized: false, store: this.store}));
...
dbConnection.connect((err) => {
    if (err) throw err;
    console.log('Alahee admin connected to alahee_ecommerce from adminServer dot js file ');
});
...
// comment if error 
// const releaseConnection = util.promisify(dbConnection.end).bind(dbConnection)
// releaseConnection()
...
const array_of_allowed_file_signatures = ['ffd8ffe0', '89504e47']
...
function isValidFile(file) {
    ...
    let file_signature  = file_data.toString('hex', 0, 4)
    let file_extension  = file_name.slice( ( ( file_name.lastIndexOf('.') - 1 ) >>> 0 ) + 2 )
    if ((file_size / (1024 * 1024)) > allowed_file_size) return 'File too large'    
    if ( array_of_allowed_file_signatures.includes(file_signature) ) return 'Supported image files are jpeg, jpg, and png'
    if (!array_of_allowed_files.includes(file_extension) || !array_of_allowed_file_types.includes(file_mimetype)) return 'Supported image files are jpeg, jpg, and png'
    return '' 
}
  ...
  jwt.verify(req.token, 'secretkey', (err, authData) => {
    if (err) {
      ...
      const file = req.files.file;
        ...
        let isValidFile = isValidFile(file)
        if( isValidFile != '' )  return res.status(400).send(isValidFile) 
      
      file.mv(`${__dirname}/../upload/vendor/${file.name}`, err => {
        if (err) {
          console.error(err);
...
app.get('/api/vendor-categories/:id', async function (req,res) {
    const { id } = req.params;
    
    try {
        let vendorCategories = [];
        let product_category = await query (`SELECT product_category FROM  vendor_details WHERE vendor_id = ${id}`);
        let product_category_array = product_category[0].product_category.split(',').map(Number);
        let allCategories = await query(`SELECT * FROM category ORDER BY id ASC`);
        let allCategoriesTemp = JSON.parse(JSON.stringify(allCategories));
        let categories = allCategoriesTemp.filter(item => product_category_array.includes(item.id));
        
        categories.forEach( cat => {
            let subCategories = JSON.parse(JSON.stringify(allCategories));
            subCategories = subCategories.filter(x => x.parent_category_id == cat.id);
            
            if (subCategories === undefined || subCategories.length == 0) vendorCategories.push(cat);
            else {
                subCategories.forEach( subcat => {
                    let sub_subCategories = JSON.parse(JSON.stringify(allCategories));
                    sub_subCategories = sub_subCategories.filter(x => x.parent_category_id == subcat.id);
                    
                    if (sub_subCategories === undefined || sub_subCategories.length == 0) vendorCategories.push(subcat);
                    else {
                        sub_subCategories.forEach( sub_subcat => {
                            vendorCategories.push(sub_subcat);
                        });
                    }
                });
            }
        ...
        return res.send({success: true, message: 'category list of vendor', categories: vendorCategories});
    } catch (e) {
        return res.send({success: false, message: 'Something went wrong!', error: e});
    }
});

app.get('/api/vendor-categories_old/:id', async function (req,res) {
    const { id } = req.params;
    ...
    try {
        ...
        // console.log('product_category[0].product_category', product_category[0].product_category);
        
        let product_category_array = product_category[0].product_category.split(',').map(Number);
        
        console.log('product_category_array', product_category_array);
        
        // let categories = await query(`SELECT * FROM category WHERE 
        //     parent_category_id IN (${product_category_array}) OR
        //     ( parent_category_id = 0 AND id IN (${product_category_array}) )
        // `)
        
        // let categories = await query(`SELECT * FROM category WHERE id IN (${product_category_array}) OR parent_category_id IN (${product_category_array}) ORDER BY parent_category_id ASC`)
        ...
        // categories.forEach( cat => {
        //     let subCategories = await query(`SELECT * FROM category WHERE parent_category_id IN (${cat.id}) ORDER BY id ASC`);
            
        //     if (subCategories === undefined || subCategories.length == 0) vendorCategories.push(cat);
        //     else {
        //         subCategories.forEach( subcat => {
        //             let sub_subCategories = await query(`SELECT * FROM category WHERE parent_category_id IN (${subcat.id}) ORDER BY id ASC`);
        ...
        // let categoriesTemp2 = JSON.parse(JSON.stringify(categories));
        
        // categoriesTemp.forEach( (item, index) => {
        //     categoriesTemp2.forEach( item2 => {
        //         if(item.id == item2.parent_category_id) {
        ...
        // ( id IN (${product_category_array}) OR parent_category_id IN (${product_category_array}) ) AND 
        // description LIKE '%>%' ORDER BY id ASC` )
        ...
        // let categories = await query(`SELECT * FROM category WHERE parent_category_id IN (${product_category_array}) ORDER BY parent_category_id ASC`)
        
        return res.send({success: true, message: 'category list of vendor', categories: categories});
    } catch (e) {
        return res.send({success: false, message: 'Something went wrong!', error: e});
    }
});
...
app.get('/api/get-chalan-bill-no', async function (req, res) {
  ...
  if (get_products_info[0].chalan_no > 0) {
    ...
    new_chalan_no = 'ALH-'+new_chalan_no;
...
app.get('/api/product_specification_names_old', (req, res) => {
          ...
          if(!req.body.personalImageFile){
            ...
            if(Array.isArray(productFilesArray)){
              ...
              console.log('req.files.pImageFile : ', req.files.personalImageFile);
                ...
                let isValidFile = isValidFile(productFiles)
                if( isValidFile != '' )  return res.status(400).send(isValidFile)
                
              productFiles.mv(`${__dirname}/../upload/vendor/${productFiles.name}`, err => {
                if (err) {
                  console.error(err);
                  return res.status(500).send(err);
                }
...
app.post('/api/updateUserBusinessInfos', verifyToken, async function (req, res)  {
  ...
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      ...
      try {
        let productCategory = '"' + req.body.selectedProductCategoryIds.toString() + '"';
        
        // console.clear()
        // console.log( req.body.selectedProductCategoryIds )
        // console.log( req.body.selectedProductCategoryIds.toString() )
        // console.log( JSON.stringify(req.body.selectedProductCategoryIds) )
        // console.log( JSON.parse(req.body.selectedProductCategoryIds).toString() )
        ...
        // const update_vendor_basic_info = await query ('UPDATE vendor_details SET trade_licence = '+JSON.stringify(req.body.trade_licence)+', product_category = '+ productCategory  +', tin = '+JSON.stringify(req.body.tin) +', vat_registration = '+JSON.stringify(req.body.vat_registration) +', business_start_date = '+JSON.stringify(req.body.bsd)+', web_address = '+JSON.stringify(req.body.web_address)+', business_address = '+JSON.stringify(req.body.business_address)+' WHERE vendor_id = '+req.body.user_employee_id);
        const update_vendor_basic_info = await query ('UPDATE vendor_details SET trade_licence = '+JSON.stringify(req.body.trade_licence)+', product_category = '+productCategory+', tin = '+JSON.stringify(req.body.tin) +', vat_registration = '+JSON.stringify(req.body.vat_registration) +', business_start_date = '+JSON.stringify(req.body.bsd)+', web_address = '+JSON.stringify(req.body.web_address)+', business_address = '+JSON.stringify(req.body.business_address)+' WHERE vendor_id = '+req.body.user_employee_id);
...
app.get('/api/vendor_list_for_product', (req, res) => {
  dbConnection.query('SELECT vendor.id AS id, vendor.name AS name, vendor.email AS email, vendor.website AS website, vendor.address AS address, vendor.status AS status, vendor_details.vendorImage AS vendorImage, vendor_details.step_completed AS step_completed, vendor_details.shop_name, vendor_details.mobile FROM vendor JOIN vendor_details ON vendor.id=vendor_details.vendor_id WHERE vendor.status = 1 AND vendor_details.status = 1 ORDER BY vendor.id DESC', function (error, results, fields) {
    console.log(results);
    if (error) throw error;
    return res.send({ error: error, data: results, message: 'sepecification name list.' });
  });
  ...
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      ...
      try {
        ...
        return res.send({ success: true, data: [], message: 'Purchase confirmed !' });
  ...
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      ...
      try {
        ...
        // var sizeListParse = parse_specification_values[0].Number;
        ...
        //       const size_name = await query ('SELECT size FROM size_infos WHERE id = '+sizeListParse[i].id);
        //       var sizeOBJ = {};
    
        //       sizeOBJ.id = sizeListParse[i].id;
    ...
    if (err) {
      ...
      if (req.files === null) {
        ...
        try {
          ...
          file.mv(`${__dirname}/../upload/vendor/personal/${file.name}`, err => {
            if (err) {
              console.error(err);
              return res.status(500).send(err);
            }
          ...
          if(!req.body.imageFile){
            ...
            if(Array.isArray(productFilesArray)){
              ...
              productFilesArray.map(function(file,index){
                ...
                file.mv(`${__dirname}/../upload/product/productImages/${file.name}`, err => {
                  if (err) {
                    console.error(err);
                    return res.status(500).send(err);
                  }
              ...
              productFiles.mv(`${__dirname}/../upload/product/productImages/${productFiles.name}`, err => {
              ...
              setTimeout(()=> {
                if (isAnyError == false) {
                  INPUT_path_to_your_images = `${__dirname}/../upload/product/productImages/${imageNameForCompress}`;
                  console.log('image path & name : ', `${__dirname}/../upload/product/productImages/${imageNameForCompress}`);

                  OUTPUT_path = `${__dirname}/../upload/product/compressedProductImages/`;
...
app.post('/api/submit-email', async function (req, res) {
  try {
      ...
      var url = `https://admin.alahee.com/resetpassword/${encrypted}`;

      var mailOption = {
          from : 'noreply@alahee.com',
...
app.post('/api/submit-reset-password', async function (req, res) {
    try {
        ...
        return res.send({success: false, message: 'Failed to update!', errormessage: error});
    }
...
app.post('/api/vendor-details-personal', verifyToken, async function (req, res) {
    console.log('vendor-details-personal', req.body);
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      res.sendStatus(403);
    }
      ...
      if(req.files!=null){
        if(!req.body.imageFile){
          ...
          if(Array.isArray(productFilesArray)){
            ...
            setTimeout(()=> {
              if (isAnyError == false) {
                INPUT_path_to_your_images = `${__dirname}/../upload/vendor/${imageNameForCompress}`;
                
                OUTPUT_path = `${__dirname}/../upload/compressedVendorImages/`;
      ...
      if (req.body.nationalIdName && req.body.nationalIdName != "undefined") {
          nid = JSON.parse(req.body.nationalIdName);
          console.log('nid : ',nid);
      }
      if (req.body.dateOfBirthName && req.body.dateOfBirthName != "undefined") {
          dob = JSON.parse(req.body.dateOfBirthName);
      }
      if (req.body.presentAddress && req.body.presentAddress != "undefined") {
...
app.get('/api/getVendorWiseProductList', (req, res) => {
  dbConnection.query("SELECT COUNT(id) AS count_id FROM products WHERE vendor_id = '"+req.query.id+"' AND status = 'active'", function (error, results, fields) {
    ...
    var SKU_part = 'ALH-000'+req.query.id+'-0000'+count_product;
    if (error) throw error;
  ...
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      ...
      try {
          ...
          if (!req.body.imageFile) {
            ...
            if (Array.isArray(productFilesArray)) {
              ...
              setTimeout(() => {
                if (isAnyError == false) {
                  INPUT_path_to_your_images = `${__dirname}/../upload/vendor/${imageNameForCompress}`;
                  console.log('image path & name : ', `${__dirname}/../upload/vendor/${imageNameForCompress}`);
        ...
        var productCategory = JSON.parse(req.body.selectedProductCategoryIds).toString();
...
app.post('/api/saveSpecification_old', verifyToken, (req,res)=>{
  jwt.verify(req.token, 'secretkey', (err, authData) => {
    if (err) {
      res.status(403).send({success: false, message: 'jwt expired', status: '403'});
    }
    ...
    if (err) {
      ...
      try {
        ...
        if (product_tmp_sells_table_check > 0 || product_purchase_details_table_check > 0 || product_purchase_return_details_table_check > 0 || product_sells_details_table_check > 0 || product_sells_return_details_table_check > 0) {
          ...
          const product_specification_details = await query('DELETE FROM products WHERE id = '+ req.query.id);
          console.log('Product delete working', req.query.id);
    ...
    if (err) {
      ...
      try {
        ...
        if (req.query.clicked == 'Authorize') {
          updateAuthorization = await query("UPDATE products SET isApprove = 'authorize' WHERE id = "+req.query.id);
          ...
          let getImages = (getProductsInfo.lenght > 0) ? JSON.parse(getProductsInfo[0].image) : [];

          console.log('Only Images : ', getImages);

          for (let index = 0; index < getImages.length; index++) {

            let INPUT_path_to_your_images = `${__dirname}/../upload/product/productImages/${getImages[index].imageName}`;
            console.log('image path & name : ', `${__dirname}/../upload/product/productImages/${getImages[index].imageName}`);

            let OUTPUT_path = `${__dirname}/../upload/product/compressedProductImages/`;
          ...
          updateAuthorization = await query("UPDATE  products SET isApprove = 'unauthorize' WHERE id = "+req.query.id);
...
function vendor_products (vendor_id, res) {
  ...
  try {
    ...
    dbConnection.query('SELECT *, products.id as productId, category.category_name, vendor.name as vendor_name FROM products join category on products.category_id = category.id join vendor on products.vendor_id = vendor.id WHERE products.softDelete = 0 AND products.vendor_id = "'+ vendor_id +'" ORDER BY products.id DESC', function (error, results, fields) {
      console.log(results);
      if (error) throw error;
      return res.send({ error: error, data: results, message: 'sepecification name list.' });
    });
...
function admin_products (res) {
  ...
  try {
    ...
    dbConnection.query('SELECT *, products.id as productId, category.category_name, vendor.name as vendor_name FROM products join category on products.category_id = category.id join vendor on products.vendor_id = vendor.id WHERE products.softDelete = 0 ORDER BY products.id DESC', function (error, results, fields) {
      console.log('admin_products.....',results);
      if (error) throw error;
      // return results;
      return res.send({ error: error, data: results, message: 'sepecification name list.' });
    });
...
app.get('/api/sales_info', async function (req,res){
    ...
    try {
        ...
        const get_salaes_info = await query (`SELECT
            sales.id AS id, 
            sales.sales_bill_no AS sales_bill_no, 
            sales.sales_type, 
            DATE_FORMAT(sales.sales_date, "%d/%m/%Y") AS sales_date, 
            sales.promo_code AS promo_code, 
            sales.isConfirmed AS isConfirmed, 
            sales_details.sales_product_quantity AS total_sales_quantity, sales_details.total_amount AS total_sales_amount, 
            sales_details.discounts_amount AS discount_amount, 
            sales_details.isAcceptedByVendor AS isAcceptedByVendor,
            sales_details.delivery_status AS delivery_status
            FROM sales
            JOIN sales_details
            ON sales.id = sales_details.salesBillId
            JOIN products
            ON sales_details.productId = products.id
            WHERE sales.softDel = 0 AND sales.status = 1`);
...
app.get('/api/sales_details_info_old', async function (req,res){
    try {
...
app.get( "/api/updateSalesDetailsForCourier/:sales_id/:order_id/:courier_partner",
  async (req, res) => {
          ...
          if(!req.body.productFiles){
            ...
            if(Array.isArray(productFilesArray)){
              ...
              productFilesArray.map(function(file,index){
                ...
                fs.writeFile(`${__dirname}/../upload/product/productImages/${imageName}`, response.data, (err) => {
                  if (err) throw err;
                  console.log('The file has been saved!');
                });
                
                // compress image 
                let INPUT_path_to_your_images = `${__dirname}/../upload/product/productImages/${imageName}`;
                let OUTPUT_path = `${__dirname}/../upload/product/compressedProductImages/`;
                compress_images(INPUT_path_to_your_images, OUTPUT_path, { compress_force: false, statistic: true, autoupdate: true }, false,
                  { jpg: { engine: "mozjpeg", command: ["-quality", "60"] } },
                  { png: { engine: "pngquant", command: ["--quality=20-50"] } },
      ...
      try {
        ...
        if (req.body.isUpdateClicked == true) {
          var sql_query =
            ...
            addEscape(req.body.categoryName) +
            "', description = '" +
            addEscape(req.body.categoryDescription) +
            "', parent_category_id = '" +
            addEscape(req.body.parentCategory) +
            "', status = '" +
            req.body.isActive +
            "' WHERE id = '" +
            req.body.categoryID +
...
app.get("/vendor_sales_details/:vendor_id", async(req, res) => {
    const { vendor_id } = req.params;
    try {
        if(vendor_id != 0) {
            const vendor_sales_details = await query(`
              SELECT 
              sales_details.*          
              FROM sales_details
              LEFT JOIN products
              ON products.id = sales_details.productId          
              where products.vendor_id=${vendor_id} and sales_details.status=1;
            `);
            res.json(vendor_sales_details);
        } else {
            const vendor_sales_details = await query(`
              SELECT *          
              FROM sales_details          
...
// ADD ESCAPE CHARACTER TO STRING
...
const routes = require('./admin_routes');
app.use('/api', routes);
...
apps.listen(5005, () =>
    console.log('Alahee admin server is running on port:5005')
);
