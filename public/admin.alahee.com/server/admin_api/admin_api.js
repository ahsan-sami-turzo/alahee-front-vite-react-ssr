...
function vendor_products (vendor_id, res) {
  ...
  try {
    ...
    dbConnection.query('SELECT *, category.category_name, vendor.name as vendor_name FROM products join category on products.category_id = category.id join vendor on products.vendor_id = vendor.id WHERE products.softDelete = 0 AND products.vendor_id = "'+ vendor_id +'" ORDER BY products.id DESC', function (error, results, fields) {
      console.log(results);
      if (error) throw error;
      return res.send({ error: error, data: results, message: 'sepecification name list.' });
    });
...
function admin_products (res) {
  ...
  try {
    ...
    dbConnection.query('SELECT *, category.category_name, vendor.name as vendor_name FROM products join category on products.category_id = category.id join vendor on products.vendor_id = vendor.id WHERE products.softDelete = 0 ORDER BY products.id DESC', function (error, results, fields) {
